...
class HWScoreCertificateTableView: HWBaseRefreshView {
...
//       var param = ["key":HWUserLogin.currentUserLogin().key, "type":"2", "pageNumber":"1", "pageSize":kPageCount] as NSDictionary
        
        var param: NSMutableDictionary! = NSMutableDictionary()
        param.setPObject(HWUserLogin.currentUserLogin().key, forKey: "key")
        param.setPObject("2", forKey: "type")
    ...
    {
        return HWScoreCertificateTableViewCell.setCellHeight()
    }
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        let cellIdentify = "cell"
        var cell = tableView.dequeueReusableCellWithIdentifier(cellIdentify) as? HWScoreCertificateTableViewCell
        if cell == Optional.None
        {
            cell = HWScoreCertificateTableViewCell.init(style:UITableViewCellStyle.Default, reuseIdentifier: cellIdentify)
        }
