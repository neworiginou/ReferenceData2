...
+ (NSDate *)dateSinceDate:(NSDate *)date WithInterval:(NSInteger)dayInterval
{
    return [NSDate dateWithTimeInterval:dayInterval*24*60*60 sinceDate:date];
}

+ (BOOL)isEquleDate:(NSDate *)date other:(NSDate *)otherDate
{
    NSDateFormatter *dateformat = [[NSDateFormatter alloc] init];
    [dateformat setDateFormat:@"yyyy-MM-dd"];
    NSString *dateStr = [dateformat stringFromDate:date];
    NSString *otherDateStr = [dateformat stringFromDate:otherDate];
