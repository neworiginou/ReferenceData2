...
//    self.navigationItem.leftBarButtonItem = [Utility navLeftBackBtn:self action:@selector(doBack:)];
    ...
    _usernameTF = [[UITextField alloc] initWithFrame:CGRectMake(backView.frame.origin.x + 15, CGRectGetMinY(backView.frame) + 7.5f , kScreenWidth - 70, 30)];
...
//    _usernameTF.textColor = THEME_COLOR_SMOKE;
    ...
    _passwordTF = [[UITextField alloc] initWithFrame:CGRectMake(backView.frame.origin.x + 15, CGRectGetMaxY(backView.frame) - 7.5f - 30, kScreenWidth - 70, 30)];
    _passwordTF.font = [UIFont fontWithName:FONTNAME size:15.0f];
    _passwordTF.placeholder = @"请输入卡号";
    _passwordTF.delegate =self;
#warning color
    _passwordTF.textColor = TITLE_COLOR_99;
    ...
    for (int i = 0; i<3; i++) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(backView1.frame) + 15, CGRectGetMinY(backView1.frame) + 7.5f + i*45, self.view.frame.size.width - 70, 30)];
...
//        UIImageView *imgV = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 12.5f, 12.5f)];
        UIImageView *imgV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"arrow_next"]];
        imgV.backgroundColor = [UIColor clearColor];
        //imgV.image = [UIImage imageNamed:@"arrow_next"];
        imgV.center = CGPointMake(kScreenWidth - 15.0f - imgV.frame.size.width / 2.0f, label.center.y);
...
//        [btn setTitle:[titleArr objectAtIndex:i] forState:UIControlStateNormal];
        ...
        btn.frame = CGRectMake(CGRectGetMinX(backView1.frame) + 15, CGRectGetMinY(backView1.frame) + 7.5f + i * 45, self.view.frame.size.width, 30);
        [_mainSV addSubview:btn];
    }
    
    _blockImgV = [[UIImageView alloc] initWithFrame:CGRectMake(backView1.frame.origin.x + 15, CGRectGetMaxY(backView1.frame) + 10, 23, 23)];
    _blockImgV.image = [UIImage imageNamed:@"choose_2_1"];
...
#warning color
    infoLab.textColor = TITLE_COLOR_99;
...
//    [confirmButton setButtonOrangeStyle];
    [confirmButton setButtonRedAndOrangeBorderStyle];
...
//            [MobClick event:@"choose_bank_province"];
            ...
            [provinceVC setProvince:^(NSDictionary *dic)
            {
                ...
                lable2.text = @"开户银行所在城市";
//                lable.textColor = [UIColor blackColor];
#warning warning
                lable.text = [dic stringObjectForKey:@"province_name"];
            }];
...
//            [MobClick event:@"choose_bank_city"];
            if (Pdic==nil) {
                [Utility showToastWithMessage:@"请先选择省份" _view:self.view];
//                [Utility showToastWithMessage:@"请先选择省份" inView:self.view];
            ...
            if ([[Pdic objectForKey:@"id"] isKindOfClass:[NSNull class]]) {
                ...
                cityVC.province_id = [Pdic stringObjectForKey:@"id"];
            }
...
#warning warning
//                 lable.textColor = [UIColor blackColor];
                 lable.text = [dic stringObjectForKey:@"city_name"];
             }];
...
//        [MobClick event:@"click_set_default"];
        _blockImgV.image = [UIImage imageNamed:@"choose_2_2"];
    }
...
//    }
//    else
        if (_usernameTF.text.length < 2 || _usernameTF.text.length > 5)
    {
        [Utility showToastWithMessage:@"持卡人姓名必须为2-5个汉字" _view:self.view];
        return;
    }
    else if(Bdic==nil)
    {
        [Utility showToastWithMessage:@"请选择银行" _view:self.view];
        return;
    }
    else if (Pdic==nil)
    {
        [Utility showToastWithMessage:@"请选择省份" _view:self.view];
        return;
    }
    else if (Cdic==nil)
    {
        [Utility showToastWithMessage:@"请选择城市" _view:self.view];
        return;
    }
    else if(_passwordTF.text.length<=0)
    {
        [Utility showToastWithMessage:@"请输入银行卡号" _view:self.view];
        return;
    }
    else if (![Utility_OC isCardNo:cardNo])
    {
        [Utility showToastWithMessage:@"输入的银行卡号不符合规则" _view:self.view];
        return;
    }
...
//
    [Utility showMBProgress:self.view _message:@"添加中..."];
...
//    [dict setPObject:@"1" forKey:@"channel"];
    ...
    [manager postHttpRequest:AddCardID_V4 parameters:dict queue:nil success:^(NSDictionary *responseObject) {
        [Utility hideMBProgress:self.view];
        
        NSString *data = [responseObject stringObjectForKey:@"data"];
        if ([data isEqualToString:@"-1"])
        {
            [Utility showToastWithMessage:@"银行卡已存在" _view:self.view];
        }
        else if ([data isEqualToString:@"1"])
        {
            ...
            if (self.logic == LogicLine_GetMoney)
            {
                ...
                getMoneyVC.popToViewController  = getMoneyVC.popToViewController;
