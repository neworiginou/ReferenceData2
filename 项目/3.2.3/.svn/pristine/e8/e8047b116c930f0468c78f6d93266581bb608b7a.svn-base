...
class HWScoreOfflineTableView: HWBaseRefreshView {
...
//        var param = ["key":HWUserLogin.currentUserLogin().key, "type":"4", "pageNumber":"1", "pageSize":kPageCount] as NSDictionary
        var param: NSMutableDictionary! = NSMutableDictionary()
        param.setPObject(HWUserLogin.currentUserLogin().key, forKey: "key")
        param.setPObject("4", forKey: "type")
    ...
    {
        return HWScoreOfflineTableViewCell.setCellHeight()
    }
    
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        let cellIdentify = "cell"
        var cell = tableView.dequeueReusableCellWithIdentifier(cellIdentify) as? HWScoreOfflineTableViewCell
        if cell == Optional.None
        {
            cell = HWScoreOfflineTableViewCell.init(style:UITableViewCellStyle.Default, reuseIdentifier: cellIdentify)
        }
