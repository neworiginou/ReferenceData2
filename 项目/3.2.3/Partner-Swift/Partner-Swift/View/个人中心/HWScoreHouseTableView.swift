...
class HWScoreHouseTableView: HWBaseRefreshView {
...
//        var param = ["key":HWUserLogin.currentUserLogin().key, "type":"1", "pageNumber":"1", "pageSize":kPageCount] as NSDictionary
        ...
        manager.postHttpRequest(kMyScore, parameters: param, queue: nil, success: { (responseObject) -> Void in
            ...
            NSNotificationCenter.defaultCenter().postNotificationName("kHouseScoreNotification", object: nil, userInfo: dataDic)
            ...
            {
                var scoreModel = HWScoreHouseModel(dic: detailDic as NSDictionary)
    ...
    {
        return HWScoreHouseTableViewCell.setCellHeight()
    }
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        let cellIdentify = "cell"
        var cell = tableView.dequeueReusableCellWithIdentifier(cellIdentify) as? HWScoreHouseTableViewCell
        if cell == Optional.None
        {
            cell = HWScoreHouseTableViewCell.init(style:UITableViewCellStyle.Default, reuseIdentifier: cellIdentify)
        }
        cell!.setValueOfContentView(self.baseListArr.pObjectAtIndex(indexPath.row) as HWScoreHouseModel)
