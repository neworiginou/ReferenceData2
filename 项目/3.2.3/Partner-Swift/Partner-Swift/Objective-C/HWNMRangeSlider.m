...
#import "HWNMRangeSlider.h"

@interface HWNMRangeSlider ()
{
...
@implementation HWNMRangeSlider
...
@synthesize lowLastValueStr;
@synthesize uperLastValueStr;
#pragma mark -
...
- (UIImage *)trackImage
{
    if(_trackImage==nil)
    {
        UIImage* image = [UIImage imageNamed:@"yellow_line"];
...
- (UIImage *)lowerHandleImageNormal
{
    if(_lowerHandleImageNormal==nil)
    {
        ...
        [image resizableImageWithCapInsets:UIEdgeInsetsMake(5, 5, 5, 5)];
...
- (CGRect)thumbRectForValue:(float)value image:(UIImage*) thumbImage
{
    ...
    thumbRect.size = CGSizeMake(thumbImage.size.width+10, thumbImage.size.height+10);
...
- (CGRect) touchRectForHandle:(UIImageView*) handleImageView
{
    ...
    touchRect.origin.x -= xPadding;
    touchRect.origin.y -= yPadding;
    touchRect.size.height += 2*xPadding;
    touchRect.size.width += 2*yPadding;
    return touchRect;
}
...
-(BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(_lowerHandle.highlighted)
    {
        ...
        if(!_upperHandle.highlighted || newValue<_lowerValue)
        {
            ...
            if ((newValue-[lowLastValueStr floatValue]) < 0)
            {
                if (newValue < 50 )
                {
                    [self setLowerValue:0 animated:_stepValueContinuously ? YES : NO];
                }
                else if (newValue >=50 && newValue<70)
                {
                    [self setLowerValue:50 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=70 && newValue < 90)
                {
                    [self setLowerValue:70 animated:_stepValueContinuously ? YES : NO];
                }
                else if (newValue >= 90 && newValue <110)
                {
                    [self setLowerValue:90 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >= 110 && newValue <130)
                {
                    [self setLowerValue:110 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >= 130 && newValue < 150)
                {
                    [self setLowerValue:130 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=150 && newValue <200)
                {
                    [self setLowerValue:150 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=200 && newValue <300)
                {
                    [self setLowerValue:200 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=300 && newValue <310)
                {
                     [self setLowerValue:300 animated:_stepValueContinuously ? YES : NO];
                }
                else
                {
                    [self setLowerValue:310 animated:_stepValueContinuously ? YES : NO];
                }
            ...
            else
            {
             ...
             lowLastValueStr = [NSString stringWithFormat:@"%f",newValue];
    ...
    if(_upperHandle.highlighted )
    {
        ...
        if(!_lowerHandle.highlighted || newValue>_upperValue)
        {
            ...
            if ((newValue-[uperLastValueStr floatValue]) < 0) {
                if (newValue < 50 )
                {
                    [self setLowerValue:0 animated:_stepValueContinuously ? YES : NO];
                }
                ...
                {
                    [self setUpperValue:70 animated:_stepValueContinuously ? YES : NO];
                }
                else if (newValue >= 90 && newValue <110)
                {
                    [self setUpperValue:90 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >= 110 && newValue <130)
                {
                    [self setUpperValue:110 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >= 130 && newValue < 150)
                {
                    [self setUpperValue:130 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=150 && newValue <200)
                {
                    [self setUpperValue:150 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=200 && newValue <300)
                {
                    [self setUpperValue:200 animated:_stepValueContinuously ? YES : NO];
                }
                else if(newValue >=300 && newValue <310)
                {
                    [self setUpperValue:300 animated:_stepValueContinuously ? YES : NO];
                }
                else
                {
                    [self setUpperValue:310 animated:_stepValueContinuously ? YES : NO];
                }
            }
            else
            {
                if (newValue<=50)
                {
                    [self setUpperValue:50 animated:_stepValueContinuously ? YES : NO];
                }
                else if (newValue >50 && newValue<=70)
                {
                    [self setUpperValue:70 animated:_stepValueContinuously ? YES : NO];
            ...
            uperLastValueStr = [NSString stringWithFormat:@"%f",newValue];
            [self bringSubviewToFront:_upperHandle];
