...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.cardInfo = [[NSMutableDictionary alloc] init];
    }
    return self;
}
...
//                [button setButtonOrangeStyle];
                [button setButtonRedAndOrangeBorderStyle];
...
//                [button setBackgroundImage:[UIImage imageNamed:@"logoutBackground.png"] forState:UIControlStateNormal];
...
//    [self.cardInfo setValue:@"on" forKey:@"isDefaut"];
    [self.cardInfo removeObjectForKey:@"isDefaut"];
    [self.cardInfo setObject:@"on" forKey:@"isDefaut"];
    [_mainTV reloadData];
    ...
    button.titleLabel.font = [UIFont fontWithName:FONTNAME size:17.0f];
    [button setTitle:@"解除绑定" forState:UIControlStateNormal];
//    [button setBackgroundImage:[UIImage imageNamed:@"logoutBackground@2x.png"] forState:UIControlStateNormal];
#warning color
//    [button setButtonGrayStyle];
    [button setButtonRedAndOrangeBorderStyle];
    [button addTarget:self action:@selector(clickDeleteCardButton:) forControlEvents:UIControlEventTouchUpInside];
...
//    [MobClick event:@"click_default_card"];
    ...
    [manager postHttpRequest:kSetDefaultCreditCard parameters:dict queue:nil success:^(NSDictionary *responseObject) {
