...
//        {
            ...
            {
                HWSearchAreaModel *model = [itemArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:_searchTV])
    {
        ...
        if (indexPath.row == 0 && _isHas && selectedSection == 0)
        {
            ...
            [self reloadSubTableFrame];
            [self.delegate1 customerSearchView:self passZone:@"" plateId:@""];
            [self hideSearchBackView];
            return;
        ...
        {
            [self.delegate1 customerSearchView:self passValue:[NSString stringWithFormat:@"%ld",(long)indexPath.row] withIndex:[NSString stringWithFormat:@"%d",selectedSection]];
            [self hideSearchBackView];
        }
    ...
    else
    {
        if (indexPath.row == 0)
        {
            ...
            [button setTitleEdgeInsets:UIEdgeInsetsMake(0, -7, 0, 7)];
            [button setImageEdgeInsets:UIEdgeInsetsMake(0, w + 2 , 0, -(w + 2))];
            [self.delegate1 customerSearchView:self passZone:zoneId plateId:@""];
            [self hideSearchBackView];
            return;
        }
        ...
        [tableView reloadData];
        [self hideSearchBackView];
        [self.delegate1 customerSearchView:self passZone:zoneId plateId:plate.plate_id];
